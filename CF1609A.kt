...
private fun show2PowCfDT2021AUT(x:Long):Long{
    var ans=0.toLong()
    var tmp=x
    do{
        if(tmp%2==0.toLong()){
...
private fun solve() {
    val n=readLong()
    var a=readLongArray(n.toInt())
    var sumPow=0.toLong()
    for (i in a.indices){
        if(a[i]%2==0.toLong()){
            sumPow+=show2PowCfDT2021AUT(a[i])
            a[i]/=2.0.pow(show2PowCfDT2021AUT(a[i]).toDouble()).toLong()
        }
    }
    var maxOdd=0.toLong()
    var maxOddPosit=0
    ...
    a[maxOddPosit]*=2.0.pow(sumPow.toDouble()).toLong()
    var ans=0.toLong()
    for(i in a.indices){
        ans+=a[i]
    }
