...
private fun solve() {
    val (A,B)=readLongs()
    val astr=A.toString().toCharArray()
    val bstr=B.toString().toCharArray()
    val digitDifference=astr.size-bstr.size
    if(digitDifference>0){
        for(i in digitDifference until astr.size){
            val a=astr[i].toInt()
            val b=bstr[i-digitDifference].toInt()
            if(a+b-96>9){
                println("Hard")
                return
            }
        ...
        for(i in -digitDifference until bstr.size){
            val a = astr[i+digitDifference].toInt()
            val b=bstr[i].toInt()
            if(a+b-96>=10){
                println("Hard")
                return
