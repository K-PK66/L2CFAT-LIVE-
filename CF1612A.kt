...
private fun absCfE117A(x:Int):Int{
    return if(x>=0){
        x
    }
    else{
...
private fun dCfE117A(x1:Int,y1:Int,x2:Int,y2:Int):Int{
    return absCfE117A(x1-x2)+absCfE117A(y1-y2)
}
//-----------------------Solving-----------------------
private fun solve() {
    val(x,y)=readInts()
    //d(A,C) equals to |xc|+|yc|, and should be |xb|/2+|yb|/2
    //d(B,C) equals to |xc-xb|+|yc-yb|, and should be |xb|/2+|yb|/2
    //In other words, |xc-xb|+|yc-yb|=|xc|+|yc|, indicating that xc = 0.5xb, yc = 0.5yb is a group of solution that satisfies
    if(x%2==0&&y%2==0){
        println("${x/2} ${y/2}")
    }
    //If B features coordinates with only 1 odd numbers, there's no solutions
    else if(x*y%2==0 && (x%2!=0||y%2!=0)){
        println("-1 -1")
    }
    else if(x>y){
        println("${(x-y)/2} $y")
